  ...
  @if($ratetimes->count())
                @foreach($ratetimes as $key=>$ratetime)
                    <tr>
                        ...
                        <td>
                            <a href="{!! url('admins/editRateTime',$ratetime->id) !!}" type="button" class="btn btn-primary ">
                        ...
                        <form action="{!! url('admins/viewRateTime',[$ratetime->id]) !!}" method="POST">
                                @csrf
                                {!! method_field('DELETE') !!}
                                <button type="submit" class="btn btn-danger "><i class="fa fa-trash-o" style="font-size:20px">
                                </i> Delete</button>
                            </form>
                        </td>
                    </tr>
